...
@register_env("PlaceSphere-v2", max_episode_steps=150)
class PlaceSphereV2Env(BaseEnv):
    SUPPORTED_ROBOTS = ["panda", "panda_placesphere"]
    ...
    def __init__(self, *args, robot_uids="panda_placesphere", robot_init_qpos_noise=0.02, **kwargs):
        print('Using robot ', robot_uids)
    ...
    def _default_sensor_configs(self):
        pose_x = sapien_utils.look_at(eye=[0.35, -0.00, 0.2], target=[0.0, -0.00, 0.1])
        camera_x = CameraConfig("base_camera_x", pose_x, 128, 128, 1, 0.01, 100)
        return [camera_x]

    @property
    def _default_human_render_camera_configs(self):
        pose = sapien_utils.look_at([0.35, -0.00, 0.2], [0.0, -0.00, 0.1])
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            xyz = torch.zeros((b, 3))
            xyz[..., 0] = (torch.rand((b, 1)) * 0.05 - 0.05)[
                ..., 0
            ]
            xyz[..., 1] = (torch.rand((b, 1)) * 0.1 - 0.05)[
                ..., 0
            ]
    ...
    def evaluate(self):
        ...
        is_obj_up = pos_obj[..., 2] > (self.radius * 3)
        return {
            "is_obj_up": is_obj_up,
    ...
    def _get_obs_extra(self, info: Dict):
        sphere = self.obj  # the sphere actor
        obb = get_actor_obb(sphere)  # trimesh OBB (a cube in this case)

        # top-down grasp
        approaching = np.array([0, 0, -1])  # along -Z
        closing = self.agent.tcp.pose.to_transformation_matrix()[0, :3, 1].cpu().numpy()
        ...
        grasp_pose = self.agent.build_grasp_pose(approaching, closing, sphere.pose.sp.p)
